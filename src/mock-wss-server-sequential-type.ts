...
type ExpiryMap = Record<string, StockItem[]>;     
...
function getTuesdaysOfMonth(year:any , monthIndex :any , day:any ) {
...
function isAfterToday(dateObj:any ) {
...
function compareWithToday(dateStr:any) {
 ...
 */
function getShortYYMMDD(dateInput :any ) {
 ...
 */
function getShortYYMMDDDigits(dateInput :any ) {
...
function formatTuesday(date :any  ) {
...
const isLastMonthofYear:boolean = currentMonthIndex ==11 ?  true : false; 
const caculateTuesdayOfNextMonth = (isLastMonthofYear :boolean  ) : any [] | undefined => {
...
let addTue = caculateTuesdayOfNextMonth(isLastMonthofYear);
   tuesdays = tuesdays.concat( addTue ? addTue :[] );

console.log("All Tuesdays:", tuesdays.map(formatTuesday));
console.log("First Tuesday of October 2025:", formatTuesday(tuesdays[0]));
 // === 4. Trade generator ===
  function generateTrade(id :any  , k_const :any ) {
    return [
      id,
...
let total_array_expiries:any  = [] ;
let total_array_expiries_truedata:any  = [] ;
/**
...
function generateTuesdayTrades(
  ...
  tuesdayObjects.forEach((exp:any) => {
    const trades = [];
...
function extractExpiryStrikeMap(dataEntries :any, isFyers :any) {
  const map  :any = {};
...
const extractExpiryFromSymbol = (symbol :any ) => {
...
let expiryKeyMap :any = {};

Object.entries(total_array_expiries as ExpiryMap).forEach(([_, fyersArr]) => {
  ...
  Object.entries(total_array_expiries_truedata as ExpiryMap ).forEach(
    ([trueKey, trueArr]) => {
...
Object.entries(total_array_expiries  as ExpiryMap ).forEach(([_, fyersArr]) => {
  ...
  const fyersSymbol = fyersArr[0].symbol;
  const fyersExpiry = extractExpiryFromSymbol(fyersSymbol ?? "");

  if (!fyersExpiry) return;

  Object.entries(total_array_expiries_truedata  as ExpiryMap).forEach(([_, trueArr]) => {
    if (!trueArr?.length) return;

    const trueSymbol = trueArr[0].symbol;
    const trueExpiry = extractExpiryFromSymbol(trueSymbol ?? "");

    if (!trueExpiry) return;

    // VALID date comparison
    if (trueExpiry.getTime() === fyersExpiry.getTime()) {
      expiryKeyMap[fyersExpiry.toISOString()] = trueExpiry.toISOString();
    }
  });
});
...
function buildSymbolLookup(total_array_expiries :any   ) {
  const map = new Map();
...
function resolveSymbols(symbols :any , total_array_expiries:any ) {
  ...
  constructor(server: HttpServer) {
     if( server !== undefined){ 
        ...
        this.wss = new WebSocketServer({ server });
          ...
          console.log("Pass a proper instance of HttpServer ")
     }
    
    this.wss?.on("connection", (ws:any, req: HttpQueryRequest) => {
     console.log("📡 Indices client connected");
     ...
     let initialtrade: Array<[string, ...unknown[]]> =[]; let idSym : Array<[string | number, string]> = []; 
 ...
 */
function sendDelayedTrades(contracts:any, index = 0) {
  ...
  ws.on("message", (msg:any) => {
     ...
     */
     ...
     // 2. Use flatMap to iterate through the nested structure and create a single flat array.
       matching_contracts =  total_array_expiries.flatMap((expiryGroup :StockItem []) => {  // total_array_expiries_truedata.flatMap(expiryGroup => {
          // expiryGroup is in the format: ["expiryDate", [contract_objects...]] // consuming from self not truedata 
          const optionsArray = expiryGroup ; // expiryGroup[1];
          
          // Filter the optionsArray: keep only elements where the symbol is in our Set.
          return optionsArray.filter((option :StockItem ) => symbolSet.has(option.symbol));
      });
      ...
      const requestedSymbols = new Set(
               request.symbols.filter((s:string) => s.startsWith("NIFTY") && s.includes("CE") || s.includes("PE"))
            )     ;
      //Build a symbol → record index from total_array_expiries
      const fyersSymbolIndex:any = {};

      Object.values(total_array_expiries).forEach((expiryArr:any) => {
        expiryArr.forEach((record:any) => {
          fyersSymbolIndex[record.symbol] = record;
        });
      }); 
      const matchedFyersRecords  : any  = [];

        requestedSymbols.forEach((symbol:any  )  => {
          ...
          console.log("\n--- Matching Contracts Using considering request symbol in  'NIFTY25D2325800PE' format : lookup in total_array_expiries ---");
           let mergeMatchedRecordSimple :any  = [];
        ...
        matching_contracts.forEach(({ id , symbol, k } : MatchingContract ) => {
      ...
      "0","0","0","0","0"
        ...
        const  nifty50  : [string, ...unknown[]] = ["NIFTY-50" , ...nifty50SpotTrade ]; 
      //      ["NIFTY-50","753989892","2025-12-16T10:12:41.627Z","141.23","0","0","0","108.92","111.05","163.63","143.79","7019403","8814904","0","0","0","0","0"]
        ...
        const uniqueMerged: Array<[string, ...unknown[]]> = Array.from(
             new Map(initialtrade.map(trade => [trade[0], trade])).values()
        );
...
{ id: "302480001", symbol: "NIFTY26J2725400CE", k: 12 },
  ...
  const tradeInterval = setInterval(() => {
    ...
    if (Array.isArray(ws.matching_contracts) && ws.matching_contracts.length > 0) {
      ws.matching_contracts.forEach(({ id, symbol, k } :MatchingContract) => {
        if (ws.subscribedSymbols.has(symbol)) {
          const trade = generateTrade(id, k);
          ws.send(JSON.stringify({ trade }));
          console.log(`[WSS] Sent trade for ${symbol}:`, trade[2]);
...
//export const indicesWSSSocket  =   new IndicesSocket( new HttpServer );// new WebSocket.Server({ server });


export   { IndicesSocket } ;
interface MatchingContract {
  id: string | number;
  symbol: string;
  k: unknown;
}
 let  matching_contracts :any = [];
